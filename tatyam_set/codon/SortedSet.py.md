...
data:
  ...
  code: "# https://github.com/tatyam-prime/SortedSet/blob/main/codon/SortedSet.py\n\
    import math\nfrom bisect import bisect_left, bisect_right\nfrom typing import\
    \ ClassVar, Generator, Optional\n\nclass SortedSet[T]:\n    size: int\n    a:\
    \ list[list[T]]\n    BUCKET_RATIO: ClassVar[int] = 16\n    SPLIT_RATIO: ClassVar[int]\
    \ = 24\n\n    def __init__(self) -> None:\n        self.size = 0\n        self.a\
    \ = []\n\n    def __init__(self, a: Generator[T]) -> None:\n        self.__init__(list(a))\n\
    \n    def __init__(self, a: list[T]) -> None:\n        \"Make a new SortedSet\
    \ from a list. / O(N) if sorted and unique / O(N log N)\"\n        n = len(a)\n\
    \        if any(a[i] > a[i + 1] for i in range(n - 1)):\n            a.sort()\n\
    \        if any(a[i] >= a[i + 1] for i in range(n - 1)):\n            a, b = [],\
    \ a\n            for x in b:\n                if not a or a[-1] != x:\n      \
    \              a.append(x)\n        n = self.size = len(a)\n        num_bucket\
    \ = int(math.ceil(math.sqrt(n / self.BUCKET_RATIO)))\n        self.a = [a[n *\
    \ i // num_bucket : n * (i + 1) // num_bucket] for i in range(num_bucket)]\n\n\
  ...
  verifiedWith: []
documentation_of: tatyam_set/codon/SortedSet.py
layout: document
redirect_from:
- /library/tatyam_set/codon/SortedSet.py
- /library/tatyam_set/codon/SortedSet.py.html
