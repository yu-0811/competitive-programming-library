...
本家ACLドキュメント : https://github.com/atcoder/ac-library/blob/master/document_ja/convolution.md
"""
...
class FFT():
    ...
    def convolution(self, a : list[int], b : list[int]) -> list[int]:
        ...
        わかりやすくいうと、インデックスの和が i になるような a, b の積の総和
        ex. a = [1, 2, 3], b = [4, 5, 6]
        res = [1*4, 1*5 + 2*4, 1*6 + 2*5 + 3*4, 2*6 + 3*5, 3*6] 
        = [4, 13, 28, 27, 18]
        (インデックスでいうと、res[0] = a[0]*b[0], res[1] = a[0]*b[1] + a[1]*b[0], ...)
        
        時間計算量
        L = |a| + |b| として、O(L log L + log mod)
        """
        n=len(a);m=len(b)
        if not(a) or not(b):
            return []
